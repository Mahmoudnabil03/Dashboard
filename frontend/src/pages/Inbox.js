...
function InboxItem({ item, onUpdated }) {
  ...
  return <div className={`p-5 ${item.replied ? 'bg-[var(--bg-elevated)]/50' : ''}`}><div className="flex gap-4"><div className="w-10 h-10 rounded-full bg-[#d4c7ff] text-[var(--error)] flex items-center justify-center font-semibold shrink-0">{(item.author || '?').slice(0, 1).toUpperCase()}</div><div className="flex-1 min-w-0"><div className="flex flex-wrap justify-between gap-2"><div><span className="font-semibold text-[var(--text-primary)]">{item.author || 'Anonymous'}</span><span className="ml-2 text-xs text-[#714cb6] capitalize">{item.platform || 'social'}</span></div><span className="text-xs text-[var(--text-tertiary)]">{item.created_at ? new Date(item.created_at).toLocaleString() : ''}</span></div><p className="text-[var(--text-primary)] mt-2">{item.content}</p>{item.post_content && <p className="text-xs text-[var(--text-tertiary)] mt-1 truncate">On: {item.post_content}</p>}<div className="flex flex-wrap items-center gap-2 mt-4">{item.replied ? <span className="text-xs text-emerald-700 flex items-center"><Check size={14} className="mr-1" /> Replied</span> : <><button onClick={generate} className="px-3 py-1.5 rounded-lg bg-[#d4c7ff] text-[var(--error)] text-sm flex items-center"><Bot size={15} className="mr-1.5" />Draft with AI</button><span className="text-xs text-[var(--text-tertiary)] flex items-center"><MessageCircle size={14} className="mr-1" />Needs reply</span></>}</div>{!item.replied && reply && <div className="mt-3 flex gap-2"><input value={reply} onChange={(event) => setReply(event.target.value)} className="flex-1 px-3 py-2 border border-[var(--border-subtle)] rounded-lg text-sm" /><button onClick={send} disabled={sending} className="px-4 py-2 rounded-lg bg-[#421d24] text-white text-sm disabled:opacity-50">{sending ? 'Sending...' : 'Send'}</button></div>}</div></div></div>;
}
