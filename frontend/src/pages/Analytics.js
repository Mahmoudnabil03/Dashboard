...
export default function Analytics() {
  ...
  const stats = [
    { label: 'Posts created', value: filteredPosts.length, icon: Send, color: 'text-blue-600 bg-blue-50' },
    { label: 'Published', value: published, icon: CheckCircle2, color: 'text-emerald-600 bg-emerald-50' },
    { label: 'Publishing rate', value: `${publishRate}%`, icon: TrendingUp, color: 'text-violet-600 bg-violet-50' },
    { label: 'Open conversations', value: openInbox, icon: MessageSquare, color: 'text-amber-600 bg-amber-50' },
  ];
  ...
  return (
    <div>
      <div className="flex flex-wrap justify-between items-start gap-4 mb-8">
        <div>
          <h1 className="text-3xl font-[460] text-gray-800">Analytics</h1>
          <p className="text-gray-500 mt-1">Understand publishing activity across your connected channels.</p>
        </div>
        <select value={period} onChange={(event) => setPeriod(event.target.value)} className="px-3 py-2 bg-white border border-gray-200 rounded-lg text-sm text-gray-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-8">
        {stats.map(({ label, value, icon: Icon, color }) => (
          <div key={label} className="bg-white border border-gray-100 rounded-2xl p-5">
            <div className="flex items-center justify-between"><div><p className="text-sm text-gray-500">{label}</p><p className="text-3xl font-[460] text-gray-800 mt-1">{value}</p></div><div className={`p-3 rounded-xl ${color}`}><Icon size={22} /></div></div>
          </div>
        ))}
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <section className="lg:col-span-2 bg-white border border-gray-100 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-5"><h2 className="text-lg font-semibold text-gray-800 flex items-center"><BarChart3 size={19} className="mr-2 text-violet-600" /> Platform overview</h2><span className="text-sm text-gray-500">{accounts.length} connected</span></div>
          {platformRows.length === 0 ? <p className="text-gray-500 py-12 text-center">Create posts to see platform analytics.</p> : <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-gray-100 text-xs uppercase text-gray-400"><th className="py-3">Platform</th><th className="py-3">Posts</th><th className="py-3">Published</th><th className="py-3">Scheduled</th><th className="py-3">Drafts</th></tr></thead><tbody>{platformRows.map((row) => <tr key={row.platform} className="border-b border-gray-50 last:border-0 text-sm"><td className="py-4 font-medium text-gray-800 capitalize">{row.platform}</td><td className="py-4 text-gray-600">{row.posts}</td><td className="py-4 text-emerald-600">{row.published}</td><td className="py-4 text-amber-600">{row.scheduled}</td><td className="py-4 text-gray-500">{row.draft}</td></tr>)}</tbody></table></div>}
        </section>
        <section className="bg-white border border-gray-100 rounded-2xl p-6">
          <h2 className="text-lg font-semibold text-gray-800 flex items-center mb-5"><Clock3 size={19} className="mr-2 text-amber-600" /> Workflow health</h2>
...
function Metric({ label, value, color }) {
  return <div><div className="flex justify-between text-sm mb-2"><span className="text-gray-600">{label}</span><span className="font-semibold text-gray-800">{value}%</span></div><div className="h-2 rounded-full bg-gray-100 overflow-hidden"><div className={`h-full rounded-full ${color}`} style={{ width: `${value}%` }} /></div></div>;
}
